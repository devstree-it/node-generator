...
const createService = {
  ...
  handler: async (argv) => {
    ...
    if (version === "es6") {
      ...
      templateArray = await import(
        `../templates/${version}/service_class/templateArray.js`
      );
      console.log(
        "🚀 ~ file: createService.js:42 ~ handler: ~ templateArray:",
        templateArray
      );
    } else {
      templateArray = await import(
        `../templates/${version}/service/templateArray.js`
      );
    }
    const serviceNameFirstCap =
      argv.name.charAt(0).toUpperCase() + argv.name.slice(1);
